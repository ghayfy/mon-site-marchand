...
async function sendCsvForOrder(req, res, id) {
  ...
  try {
    ...
    // Séparateur puis lignes produits
    lines.push('');
    ...
    for (const it of items) {
      lines.push([
        csvEscape(it.title),
        csvEscape(it.qty),
        csvEscape(Number(it.price_h_t).toFixed(2)),
        csvEscape(Number(it.tva).toFixed(2)),
      ].join(','));
    }
...
router.get('/admin/orders/:id/export.csv', (req, res) => {
  return sendCsvForOrder(req, res, req.params.id);
});
router.get('/admin/orders/:id.csv', (req, res) => {
  return sendCsvForOrder(req, res, req.params.id);
});

// (Optionnel) Stub PDF propre pour éviter 404/400
router.get('/admin/orders/export.pdf', (req, res) => {
  res.type('application/pdf').status(200).send('%PDF-1.4\n% Export PDF non implémenté\n');
});
router.get('/admin/orders/:id/export.pdf', (req, res) => {
  res.type('application/pdf').status(200).send('%PDF-1.4\n% Export PDF non implémenté\n');
});
router.get('/admin/orders/:id.pdf', (req, res) => {
  res.type('application/pdf').status(200).send('%PDF-1.4\n% Export PDF non implémenté\n');
});
