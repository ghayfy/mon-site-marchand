...
DB_PASS=${DB_PASS:-app}
run_mysql() {
  docker compose exec -T db sh -lc "MYSQL_PWD='$DB_PASS' mysql -u'$DB_USER' -D '$DB_NAME' -N -B -e \"$1\""
}
docker compose exec -T db sh -lc "
  MYSQL_PWD='$DB_PASS' mysql -u'$DB_USER' -D '$DB_NAME' -e \"
...
for f in $(ls -1 db/migrations/*.sql 2>/dev/null | sort); do
  ...
  if [ "$applied" = "0" ]; then
    echo "==> applying $base"
    docker compose exec -T db sh -lc "MYSQL_PWD='$DB_PASS' mysql -u'$DB_USER' -D '$DB_NAME' < /dev/stdin" < "$f"
